...
/** Same "only show what's real" convention as the Footer's social row. */
const SOCIAL_ICONS = [
...
/** Persistent desktop contact dock (hidden on mobile, where the sticky bar takes over). */
export function FloatingWhatsApp() {
  const activeSocials = SOCIAL_ICONS.filter((s) => s.url)

  return (
    <div className="fab-dock" aria-label="Quick contact">
      {activeSocials.map(({ key, url, Icon, label }) => (
        <a
          key={key}
          className="fab-dock__item fab-dock__item--social"
          href={url}
          target="_blank"
          rel="noopener noreferrer"
          aria-label={label}
          onClick={() => track('social_click', { network: key, source: 'floating' })}
        >
      ...
      <a
        className="fab-dock__item fab-dock__item--call"
        href={telUrl(CONTACT.office.phone)}
        aria-label={`Call ${CONTACT.office.display}`}
        onClick={() => track('phone_click', { source: 'floating' })}
      >
        <Phone aria-hidden="true" />
      </a>

      <a
        className="fab-dock__item fab-dock__item--whatsapp"
      ...
      >
        <WhatsAppIcon />
        <span className="fab-dock__pulse" aria-hidden="true" />
      </a>
    </div>
  )
}
